...
function buildTaskEmailHtml(
  ...
  const priorityChip = task.priority
    ? (() => {
        const [bg, fg] = priorityColors[task.priority.toLowerCase()] || ["#e8edf2", "#1b1f24"];
        return `<span style="${chipStyle(bg, fg)}">${escapeHtml(task.priority)}</span>`;
      })()
    : "";
  const statusChip = task.status
    ? (() => {
        const [bg, fg] = statusColors[task.status.toLowerCase()] || ["#e8edf2", "#1b1f24"];
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:540px;background:#ffffff;border:1px solid #E7DCCB;border-radius:16px;overflow:hidden;">
  ...
  <tr><td style="padding:18px 30px 0;">
    <div style="font-size:20px;font-weight:600;color:#1b1f24;">Task assigned to you</div>
  ...
  <tr><td style="padding:18px 30px 0;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#F3ECE0;border:1px solid #E7DCCB;border-radius:20px;">
      <tr><td style="padding:16px 18px;">
        <p style="margin:0 0 8px;font-size:17px;font-weight:600;color:#1b1f24;">${escapeHtml(task.title)}</p>
  ...
  <tr><td style="padding:24px 30px;" align="center">
    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto;"><tr>
      <td style="padding:0 6px 0 0;"><a href="${completeUrl}" style="display:inline-block;padding:11px 26px;background:#0f7490;color:#ffffff;font-size:13px;font-weight:600;border-radius:10px;text-decoration:none;">Mark completed</a></td>
      <td style="padding:0 0 0 6px;"><a href="${appUrl}" style="display:inline-block;padding:11px 26px;background:#ffffff;border:1px solid #0f7490;color:#0f7490;font-size:13px;font-weight:600;border-radius:10px;text-decoration:none;">View in Focus OS</a></td>
    </tr></table>
  </td></tr>
