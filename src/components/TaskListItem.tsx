...
export const TaskListItem = ({ task, onUpdate, onEditTask, onEditTaskImages, onEditTaskDates, onAssignTask, onRequestChanges, onDismissChangeRequest, onDeleteTask, globalViewMode, isIndividuallyExpanded, onTaskClick, projects = [], scopeLabel }: TaskListItemProps) => {
  ...
  if (globalViewMode === 'minimal') {
    ...
    return (
      ...
      <div
        data-task-card
        className={`group w-full glass-card rounded-lg px-1.5 py-1 hover:border-primary/50 cursor-pointer ${timer.isRunning ? 'border-glow-pulse' : ''} ${isFading ? 'animate-fade-out' : ''}`}
        onClick={onTaskClick}
      >
  ...
  return (
    <>
      <div 
        data-task-card
        className={`group w-full glass-card rounded-lg p-1.5 hover:border-primary/50 cursor-pointer ${timer.isRunning ? 'border-glow-pulse' : ''} ${isFading ? 'animate-fade-out' : ''}`}
